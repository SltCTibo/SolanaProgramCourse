...
const ANCHOR_SLOW_TEST_THRESHOLD = 20 * SECONDS;
...
describe("swap", async () => {
  ...
  const tokenBWanterAmount = new BN(1_000_000);
  ...
  before("Creates Alice and Bob accounts, 2 token mints, and associated token accounts for both tokens for both users", async () => {
    ...
    accounts.makerTokenAccountB = bobTokenAccountB;
    accounts.takerTokenAccountB = aliceTokenAccountB;
  })
  ...
  it("Puts the tokens Alice offers into the vault when Alice makes an offer!", async () => {
    ...
    const transactionSignature = await program.methods
      .makeOffer(offerId, tokenAOfferedAmount, tokenBWanterAmount)
      .accounts({ ...accounts })
      .signers([alice])
      .rpc();
    ...
    assert(offerAccount.tokenBWantedAmount.eq(tokenBWanterAmount));
  ...
  it("Puts the tokens from the vault into Bob's account, and gives Alice Bob's tokens, when Bob takes an offer", async () => {
    ...
    assert(aliceTokenAccountBalanceAfter.eq(tokenBWanterAmount));
  }).slow(ANCHOR_SLOW_TEST_THRESHOLD);
});
