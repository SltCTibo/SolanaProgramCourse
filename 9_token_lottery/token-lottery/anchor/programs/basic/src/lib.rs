...
declare_id!("6PxCKKwFioF8WvWECkAD4qGHAFaGPesBHFVAR7ozo4Br");
...
#[constant]
pub const URI: &str = "https://raw.githubusercontent.com/SltCTibo/SolanaProgramCourse/main/9_token_lottery/metadata.json";
...
pub mod token_lottery {
        ...
        set_and_verify_sized_collection_item(
            CpiContext::new_with_signer(
                ...
                SetAndVerifySizedCollectionItem {
                    ...
                    payer: ctx.accounts.payer.to_account_info(),
                    update_authority: ctx.accounts.collection_mint.to_account_info(),
                    collection_metadata: ctx.accounts.colletion_metadata.to_account_info(),
    ...
    pub fn reveal_winner(ctx: Context<RevealWinner>) -> Result<()> {
        ...
        if ctx.accounts.randomness_account.key() != token_lottery.randomness_account {
            return Err(ErrorCode::IncorretRandomnessAccount.into())
        }
...
pub struct BuyTicket<'info> {
    ...
    #[account(
        mut,
        seeds = [
            b"metadata", token_metadata_program.key().as_ref(), collection_mint.key().as_ref(),
        ],
        bump,
        seeds::program = token_metadata_program.key(),
    )]
    /// CHECK: This account will be initialized by the metaplex program
    pub colletion_metadata: UncheckedAccount<'info>,
