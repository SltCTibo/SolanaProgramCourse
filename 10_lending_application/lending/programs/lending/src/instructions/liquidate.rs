...
use crate::{borrow::calculate_accrued_interest, constants::{MAX_AGE, SOL_USD_FEED_ID, USDC_USD_FEED_ID}, error::ErrorCode, state::{Bank, User}};
...
pub fn process_liquidate(ctx: Context<Liquidate>) -> Result<()> {
    ...
    let price_update = &mut ctx.accounts.price_update_account;
